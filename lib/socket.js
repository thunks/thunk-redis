...
function createSocket(redis, options, socketState) {
  ...
  return socket
    ...
    .on('data', function(chunk) {
      ...
      if (!reply.resp) reply.resp = initResp(redis, socket, reply);
      reply.resp.feed(chunk);
    })
    .on('error', function(error) {
      flushAll(redis, this, error);
      redis.emit('error', error);
    })
    .on('close', function(hadError) {
      flushAll(redis, this, new Error(this.redisId + ' was closed'));
      unmountSocket(redis, socket, hadError);
    })
    .on('timeout', function() {
      // var error = new Error('The redis connection was timeout');
      // redis.emit('timeout');
      // if (socketState.attempts <= redis._redisState.options.maxAttempts) {
      //   flushPending(socket, error);
      //   socket.removeAllListeners();
      //   socket.destroy();
      //
      //   redis.emit('reconnecting', {
      //     delay: redis._redisState.options.retryDelay,
      ...
      //     });
      //   }, redis._redisState.options.retryDelay);
      // } else {
      //   flushAll(redis, socket, error);
      //   redis.clientEnd();
      // }
    })
    .on('end', function() {
      flushAll(redis, this, new Error(socket.redisId + ' was ended'));
  ...
  Thunk(function(callback) {
    ...
    return function(callback) {
      var command = createCommand(redis, ['info', 'default'], function(error, res) {
        ...
        res = res.toString();
        if (res.indexOf('cluster_enabled:1') > 0) redisState.clusterMode = true;
        if (res.indexOf('role:master') < 0) socket.redisMaster = false;
        else {
          socket.redisMaster = true;
          // set default socket for redis
          if (!redisState.slots[-1]) redisState.slots[-1] = socket.redisId;
        }
        callback();
      });
      socket.commandQueue.push(command);
    ...
    if (!redisState.clusterMode) return;
    return function(callback) {
    ...
    assignCommands(redis);
  });

  execQueue(socket, redisState);
}

function initResp(redis, socket, reply) {
  var redisState = redis._redisState;
...
function sendCommand(redis, command, args, additionalCallbacks) {
  return Thunk.call(redis, function(callback) {
    ...
    this._redisState.commandQueue.push(createCommand(redis, args, callback, additionalCallbacks));
    assignCommands(this);
  });
}

function assignCommands(redis) {
  var redisState = redis._redisState;
  if (!redisState.connected) return;

  var count = redisState.commandQueue.length;
  var assignedSockets = Object.create(null);
  while (count--) {
    var command = redisState.commandQueue.shift();
    ...
    if (!assignedSockets[redisId]) assignedSockets[redisId] = socket;
    socket.commandQueue.push(command);
  }
...
function execQueue(socket, redisState) {
...
function flushAll(redis, socket, error) {
  flushPending(socket, error);
...
function unmountSocket(redis, socket, hadError) {
  ...
  if (!sockets.length) return redis.clientEnd(hadError);
