...
function initSocket(redis, options, socketState) {
  ...
  socket
    ...
    .on('data', function (chunk) {
      ...
      if (socket.debugMode) tool.log({reply: reply, socketChunk: chunk});
...
function execQueue(redis, init) {
  ...
  else socket.pendingWatcher = {commands: []};
  ...
  while (socket.commandQueue.length && count--) {
    ...
    pendingWatcher.commands.push({name: command.name, callback: command.callback});
    while (command.additionalCallbacks--)
      pendingWatcher.commands.push({name: command.name, callback: noop});
...
var subReplyTypes = {subscribe: true, psubscribe: true};

function isSubReply(reply) {
  return reply && subReplyTypes.hasOwnProperty(reply[0]);
}

var unSubReplyTypes = {unsubscribe: true, punsubscribe: true};
