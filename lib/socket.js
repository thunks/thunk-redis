...
function execQueue(socket, init) {
  if (!init && (!socket.redisConnected || socket.pendingWatcher)) return;
  ...
  socket.pendingWatcher = {commands: commands};

  if (socket.frequency[commands.length]) socket.frequency[commands.length]++;
  else socket.frequency[commands.length] = 1;
