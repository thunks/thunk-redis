...
function initSocket(options) {
  ...
  on('data', function (chunk) {
    ...
    try {
      ...
      if (util.isError(reply)) throw reply;
    } catch (error) {
  ...
  on('error', function (error) {
    ...
    if (!redis.listeners('error').length) throw error;
...
function redisReady() {
  this.socket.redisConnected = true;
...
function checkQueue() {
  if (!this.redisConnected || this.pendingCommand || !this.commandQueue.length) return;
