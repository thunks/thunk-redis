...
Connection.prototype.reconnecting = function(error) {
  ...
  if (this.redis._redisState.ended || this.ended) return;
...
Connection.prototype.checkConnection = function() {
  ...
  Thunk(function(callback) {
    ...
    return redisState.clusterMode && function(callback) {
      var command = createCommand(ctx.redis, ['cluster', 'slots'], function(error, res) {
        ...
        tool.each(res, function(info) {
          ...
          // get other nodes.
          var _connection = createConnection(ctx.redis, replicationIds[0]);
...
Connection.prototype.tryRemove = function(hadError, tryEnd) {
  ...
  if (hadError) this.redis.emit('error', hadError);
  if (tryEnd && !connectionIds.length) return this.redis.clientEnd(hadError);
};
