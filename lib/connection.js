...
function sendCommand (redis, commandName, args, additionalCallbacks, responseHook) {
  return thunk.call(redis, function (callback) {
    const command = createCommand(this, commandName, args, callback, additionalCallbacks, responseHook)
...
function createConnection (redis, id) {
  const redisState = redis._redisState
...
function Connection (redis, id) {
  ...
  this.evalshaP = Object.create(null)
}
...
Connection.prototype.rescuePending = function () {
  while (this.pendingQueue.length) {
    const command = this.pendingQueue.pop()
...
Connection.prototype.disconnect = function () {
  ...
  const redisState = this.redis._redisState
...
Connection.prototype.connect = function () {
  ...
  const address = unwrapAddress(this.id)
  const options = this.redis._redisState.options
...
Connection.prototype.reconnecting = function () {
  const ctx = this
  const redisState = this.redis._redisState
  const options = redisState.options
  this.connected = false
  ...
  if (this.attempts <= options.maxAttempts) {
    ...
    const err = new Error('Reconnect ECONNREFUSED ' + this.id)
    const address = unwrapAddress(this.id)
...
Connection.prototype.checkConnection = function () {
  ...
  redisState.thunkE(function (callback) {
    ...
    const command = createCommand(ctx.redis, 'auth', [options.authPass], callback)
    if (command) ctx.writeCommand(command)
  })(function () {
    // check replication and cluster
    return function (callback) {
      const command = createCommand(ctx.redis, 'info', ['default'], function (error, res) {
    ...
    return function (callback) {
      const command = createCommand(ctx.redis, 'select', [options.database], function (error, res) {
...
Connection.prototype.createResp = function () {
  ...
  return new Resp({bufBulk: ctx.bufBulk})
    ...
    .on('data', function (data) {
      const command = pendingQueue.first()
...
function createCommand (redis, commandName, args, callback, additionalCallbacks, responseHook) {
  ...
  const reqArray = tool.slice(args)
  reqArray.unshift(commandName)

  const _callback = !responseHook ? callback : function (err, res) {
...
function dispatchCommand (redisState, command) {
  const res = redisState.getConnection(command.slot)
