...
exports.createClient = function (port, host, options) {
  ...
  if (Array.isArray(port)) {
    options = host || {}
    addressArray = normalizeNetAddress(port, options)
...
function normalizeNetAddress (array, options) {
  return array.map(function (address) {
    if (typeof address === 'string') {
      ...
      if (address.indexOf('redis://') === 0) {
        ...
        if (obj.auth) {
          options.authPass = obj.auth
          // https://www.iana.org/assignments/uri-schemes/prov/redis
          // redis://user:secret@localhost:6379
          const i = options.authPass.indexOf(':')
          if (i >= 0) {
            options.authPass = options.authPass.slice(i + 1)
          }
        }
        return wrapIPv6Address(obj.host)
      }
      return wrapIPv6Address(address)
    }
...
exports.normalizeNetAddress = normalizeNetAddress
