...
// (Redis 2.9.104) `redis-cli command`
...
var commands = ['quit'].concat(Object.keys(commandsInfo));
...
exports.initCommands = function(proto) {
  ...
  proto.clientCalcSlot = function(reqArray) {
    if (!this._redisState.clusterMode) return -1;
    var info = commandsInfo[reqArray[0]];
    if (!info || !info[2]) return -1;

    var keyIndex = info[2];
    var slot = calcSlot(reqArray[keyIndex]);
    if (keyIndex === info[3] || !info[4]) return slot;
    keyIndex += info[4];
