...
// (Redis 3.1.999) `redis-cli command`
...
const commandsInfo = {
  ...
  eval: [-3, ['noscript', 'movablekeys'], 3, -1, 1], // changed for calc slot
  evalsha: [-3, ['noscript', 'movablekeys'], 3, -1, 1], // changed for calc slot
  ...
  geodecode: [2, ['readonly'], 0, 0, 0],
  geodist: [-4, ['readonly'], 0, 0, 0],
  geoencode: [-3, ['readonly'], 0, 0, 0],
  geohash: [-2, ['readonly'], 0, 0, 0],
  geopos: [-2, ['readonly'], 0, 0, 0],
...
exports.initCommands = function (proto) {
  ...
  proto.clientCalcSlot = function (reqArray) {
    var info = commandsInfo[reqArray[0]]
    if (!info || reqArray.length === 1 || (info[2] === 0 && info[0] !== 1)) return -1
  ...
  const scriptsSha = Object.create(null)
  proto.evalauto = function (script, numkeys, key) {
    var args = adjustArgs(tool.slice(arguments))
    script = String(args[0]).trim()

    var sha = scriptsSha[script]
    if (!sha) sha = scriptsSha[script] = genSha1(script)
    args[0] = sha

    return thunk.call(this, function (callback) {
      sendCommand(this, 'evalsha', args)(function (err, res) {
        if (err == null) return res
        if (String(err).indexOf('NOSCRIPT') === -1) throw err

        var connection = this._redisState.getConnection(calcSlot(args[2]))
        return connection.sendCommand('script', ['load', script])(function (err, res) {
          if (err) throw err
...
function genSha1 (str) {
  return crypto.createHash('sha1').update(new Buffer(str)).digest('hex')
}
