...
exports.initCommands = function (proto) {
  ...
  proto.clientCalcSlot = function (reqArray) {
    ...
    if (!info || reqArray.length === 1) return -1
    ...
    if (!info[4] || keyIndex === info[3]) return slot
    keyIndex += info[4]
    // if there one more keys, just check their slots.
    while (reqArray[keyIndex]) {
      if (calcSlot(reqArray[keyIndex]) !== slot) {
        throw new Error('Request keys are in different redis nodes.')
      }
      keyIndex += info[4]
    }
    return slot
